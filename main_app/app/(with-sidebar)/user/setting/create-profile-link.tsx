...
import { createProfileLink } from "@/app/actions/profile/profile-links";
...
export default function CreateProfileLink({
  ...
  const handleCreateLink = async () => {
    ...
    try {
      ...
      if (result.success) {
        toast.success("Profile link created successfully!");
        setOpen(false);
        setSelectedTag("");
        setVisibility("PUBLIC");
        router.refresh();
      } else {
        toast.error(result.error || "Failed to create profile link");
      }
    } catch (error) {
